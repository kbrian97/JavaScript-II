...
let fullName = [];
runners.forEach( id => {
    let result = `${id.first_name} ${id.last_name}`;
...
let allCaps = [];
runners.map(first => {
    result = first.first_name.toUpperCase();
    allCaps.push(result);
});
console.log(allCaps); 
...
runners.filter(large => {
    const i = new RegExp ('L');
    if(i.test(large.shirt_size)){
...
ticketPriceTotal = runners.reduce((donate,tallied) => {
return donate + tallied.donation;
},0);
...
runners.filter(company =>{
    ...
    if (S.test(company.company_name)){
        return Scompany.push(company);
    }
    });
...
let allLower = [];
runners.map(first => {
    result = first.first_name.toLowerCase();
    allLower.push(result);
});
